var app = (function () {
	...
	function newProject(_, projects) {
		projects.push({
			"name": "New Project",
			"id": 0,
	...
	var root_2 = add_locations(template(`<br><label><input type="checkbox"> <input class="taskName"></label>`, 1), App[FILENAME], [
		[51, 6],
		[51, 10, [[51, 17], [51, 71]]]
	]);

	var on_click = (__1, newTask, project) => newTask(get(project).id);

	var root_1 = add_locations(template(`<hr> <input class="title"> <br> <textarea class="description"></textarea> <!> <br> <button>+</button>`, 1), App[FILENAME], [
		[46, 2],
		[47, 4],
		[47, 52],
		[48, 4],
	...
	var root = add_locations(template(`<main><h1>Taskflare</h1> <!> <hr> <button>+</button></main>`), App[FILENAME], [
		[
			43,
			0,
			[[44, 2], [55, 2], [56, 2]]
	...
	function App($$anchor, $$props) {
		...
		each(node, 17, () => projects, index, ($$anchor, project, $$index_1) => {
			var fragment = root_1();
			var input = sibling(first_child(fragment), 2);
			validate_binding("bind:value={project.name}", () => get(project), () => "name");

			var textarea = sibling(input, 4);
			validate_binding("bind:value={project.description}", () => get(project), () => "description");

			var node_1 = sibling(textarea, 2);

			each(node_1, 17, () => get(project).tasks, index, ($$anchor, task, $$index) => {
				var fragment_1 = root_2();
				var label = sibling(first_child(fragment_1));
				var input_1 = child(label);
				validate_binding("bind:checked={task.completed}", () => get(task), () => "completed");

				var input_2 = sibling(input_1, 2);
				validate_binding("bind:value={task.name}", () => get(task), () => "name");
				bind_checked(input_1, () => get(task).completed, ($$value) => (get(task).completed = $$value));
				bind_value(input_2, () => get(task).name, ($$value) => (get(task).name = $$value));
				append($$anchor, fragment_1);
			});

			var button = sibling(node_1, 4);

			button.__click = [on_click, newTask, project];
			bind_value(input, () => get(project).name, ($$value) => (get(project).name = $$value));
			bind_value(textarea, () => get(project).description, ($$value) => (get(project).description = $$value));
			append($$anchor, fragment);
		});

		var button_1 = sibling(node, 4);

		button_1.__click = [newProject, projects];
		append($$anchor, main);
